...
def search_jobs(requirements):
    ...
    data = {"job_requirements": f"{requirements}"}
    ...
    if st.session_state.button1_clicked:
        ...
        with st.spinner("Buscando os melhores candidatos..."):
            ...
            if results:
                ...
                st.markdown(
                    "<h3 style='color:green;'>Busca Finalizada!</h1>",
                    unsafe_allow_html=True,
                )
                st.write("Top 5 Candidatos:")
                st.write(f"{results['result']['raw']}")
                st.write(
                    f"Tokens Usados: {results['result']['token_usage']['total_tokens']}"
                )
                st.write(
                    f"Total de requisições: {results['result']['token_usage']['successful_requests']}"
                )
